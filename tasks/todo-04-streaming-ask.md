...
- [ ] 3. Disconnect handling: check `request.is_disconnected()` between chunks and handle generator cancellation (`GeneratorExit` / anyio cancellation) → close the Anthropic stream context so token generation stops (spec §5.1)
- [ ] 4. Error mapping:
...
- [ ] 5. CORS middleware restricted exactly to `ALLOWED_ORIGINS` (comma list; dev needs both `http://localhost:8088` and `http://localhost:9000`)
- [ ] 6. Structured logging: request id, route, duration_ms, model, token usage incl. `cache_read_input_tokens` when available, error category. **No questions/answers/keys.**
- [ ] 7. Tests (mock the Anthropic client):
...
- [ ] Valid request → ordered `delta` events then `done`, with correct SSE headers
- [ ] System prompt bytes identical across requests regardless of route; route/viz_type only in the user turn
- [ ] Simulated client disconnect cancels the in-flight model stream
- [ ] Timeouts/provider failures map to `TIMEOUT` / `MODEL_UNAVAILABLE`; mid-stream failures emit a final `error` event
